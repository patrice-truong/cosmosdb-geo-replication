import { ConnectionPolicy, Container, CosmosClient, CosmosClientOptions, SqlQuerySpec } from '@azure/cosmos'
...
export class CosmosDBService {
  ...
  async storeCart(cart: Cart) {
    try {
      ...
      const { resources: items } = await this.cartsContainer.items
        .query(sqlQuerySpec)
        .fetchAll()
  ...
  async getProducts() {
    ...
    try {
      ...
      const { resources: items } = await this.productsContainer.items
        .query(sqlQuerySpec)
        .fetchAll()
